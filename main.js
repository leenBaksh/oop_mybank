...
async function bankService(bank) {
    do {
        ...
        if (service.select == "View Balance") {
            ...
            if (!account) {
                console.log(chalk.blue.bold("invalid account number"));
            }
            if (account) {
                let name = HBL.customer.find((item) => item.accNumber == account?.accnumber);
                console.log(`Dear ${chalk.magenta.italic(name?.firstName)} ${chalk.green.italic(name?.lastName)} you account balance is ${chalk.bold.blueBright(`$${account.balance}`)}`);
            }
        }
        //"Cash withdraw"
        if (service.select == "Cash withdraw") {
            ...
            if (account) {
                ...
                if (ans.rupees > account.balance) {
                    console.log(chalk.green.bold("current balance is not enough"));
                }
